        ...
        for s in sntncs:
            ...
            for w in words:
                if w not in stop:
                    ...
                    #s_nor_stp_lmt = s_nor_stp_lmt.encode('latin1')
...
def cal_tf_Idf():
    ...
    for f in files:
        ...
        for i in range(len(nor_stp_lmt)):
            ...
            for wd in words:
                try:
                    wd = wd.replace("[", "").replace("]", "").replace("(", "").replace(")", "").replace("{",
                                                                                                        "").replace(
                        "}", "")
                    r = re.compile(wd + ".*")
                except:
        ...
        file_nm = os.path.join(output_path, f + '_summary_casesummariser.txt')
        w_f = open(file_nm, "w")
        sumr = ""
        for pair in sorted_x:
            sumr = sumr + pair[0] + " "
        print('writing output to ', w_f)
        w_f.write(sumr)
        w_f.close()
        # except:
        #     print('Error')
...
if __name__ == '__main__':
    ...
    input_path = root_path + '/data/sample_judgement_text_cleaned/'

    output_path = root_path + '/tests/summariser_tests/output/'
    dict_path = root_path + '/src/text_summarizers/CaseSummarizer/dictionary.txt'
