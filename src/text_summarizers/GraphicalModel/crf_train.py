...
from src.text_summarizers.GraphicalModel import formulated_constants
import operator
import pycrfsuite
import numpy as np
...
def trainFromAnnotatedJson(file):
    ...
    training_postag = add_postag(training)
    X_train = [sentenceToFeatures(training_postag[i], para_positions[i] * 1.0 / total_paras) for i in
               range(len(training_postag))]
    Y_train = [sentenceToLabels(sentence) for sentence in training_postag]
